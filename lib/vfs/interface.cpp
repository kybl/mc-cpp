...
\
    if (!vfs_path_element_valid (path_element1) || !vfs_path_element_valid (path_element2) || \
        path_element1->clazz != path_element2->clazz) \
    { \
        errno = EXDEV; \
        return (-1); \
    } \
...
mc_opendir (const vfs_path_t * vpath)
{
    ...
    handle = vfs_new_handle (path_element->clazz, vfs_path_element_clone (path_element));
...
mc_stat (const vfs_path_t * vpath, struct stat *buf)
{
    ...
    if (vfs_path_element_valid (path_element))
    {
        result = path_element->clazz->stat ? path_element->clazz->stat (vpath, buf) : -1;
        if (result == -1)
            errno = path_element->clazz->name ? vfs_ferrno (path_element->clazz) : E_NOTSUPP;
    }
...
mc_lstat (const vfs_path_t * vpath, struct stat *buf)
{
    ...
    if (vfs_path_element_valid (path_element))
    {
        result = path_element->clazz->lstat ? path_element->clazz->lstat (vpath, buf) : -1;
        if (result == -1)
            errno = path_element->clazz->name ? vfs_ferrno (path_element->clazz) : E_NOTSUPP;
    }
...
mc_chdir (const vfs_path_t * vpath)
{
    ...
    result = path_element->clazz->chdir (cd_vpath);
    if (result == -1)
    {
        errno = vfs_ferrno (path_element->clazz);
        goto error_end;
    ...
    current_vfs = path_element->clazz;
