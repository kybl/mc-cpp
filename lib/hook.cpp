...
{
    hook_t *new_list = nullptr;
    hook_t *current, *next;

    for (current = *hook_list; current != nullptr; current = next)
    {
        next = current->next;
