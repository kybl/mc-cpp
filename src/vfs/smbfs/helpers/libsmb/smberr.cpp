...
{
    static pstring ret;
    int clazz = CVAL (inbuf, smb_rcls);
    int num = SVAL (inbuf, smb_err);
    int i, j;

    for (i = 0; err_classes[i].clazz; i++)
        if (err_classes[i].code == clazz)
        {
            if (err_classes[i].err_msgs)
            {
                ...
                for (j = 0; err[j].name; j++)
                    ...
                    {
                        if (DEBUGLEVEL > 0)
                            slprintf (ret, sizeof (ret) - 1, "%s - %s (%s)", err_classes[i].clazz,
                                      err[j].name, err[j].message);
                        else
                            slprintf (ret, sizeof (ret) - 1, "%s - %s", err_classes[i].clazz,
                                      err[j].name);
                        return ret;
                    }
            }

            slprintf (ret, sizeof (ret) - 1, "%s - %d", err_classes[i].clazz, num);
            return ret;
        }

    slprintf (ret, sizeof (ret) - 1, "Error: Unknown error (%d,%d)", clazz, num);
    return (ret);
}
