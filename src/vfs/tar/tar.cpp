    ...
    while (true)
    {
        ...
        *h_size = tar_decode_header (header, arch);
            ...
            || header->header.typeflag == GNUTYPE_LONGLINK)
        {
            ...
            g_free (*longp);
            char *bp = *longp = static_cast<char *> (g_malloc (*h_size + 1));
                  ...
                  const vfs_path_element_t * vpath_element)
{
    ...
    /* Open for reading */
    tard = tar_open_archive_int (vpath_element->clazz, vpath, archive);
    if (tard == -1)
        return -1;
    ...
    while (true)
    {
        ...
        status = tar_read_header (vpath_element->clazz, archive, tard, &h_size);
