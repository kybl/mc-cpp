...
static int
undelfs_lsdel_proc (ext2_filsys _fs, blk_t * block_nr, int blockcnt, void *private_)
{
    struct lsdel_struct *_lsd = (struct lsdel_struct *) private_;
    (void) blockcnt;
    _lsd->num_blocks++;
...
undelfs_loaddel (void)
{
    ...
    max_delarray = 100;
    num_delarray = 0;
    delarray = static_cast<deleted_info *> (g_try_malloc (sizeof (struct deleted_info) * max_delarray));
    if (!delarray)
    {
...
undelfs_opendir (const vfs_path_t * vpath)
{
    ...
    if (!undelfs_loaddel ())
        goto quit_opendir;
    vfs_print_message (_("%s: done."), path_element->clazz->name);
    return fs;
  quit_opendir:
...
static int
undelfs_dump_read (ext2_filsys param_fs, blk_t * blocknr, int blockcnt, void *private_)
{
    int copy_count;
    undelfs_file *p = (undelfs_file *) private_;
