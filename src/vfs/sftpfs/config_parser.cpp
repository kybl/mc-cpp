...
sftpfs_fill_connection_data_from_config (struct vfs_s_super *super, GError ** mcerror)
{
    ...
    sftpfs_super->config_auth_type |= (config_entity->identities_only) ? 0 : SFTPS_AUTH_AGENT;
    sftpfs_super->config_auth_type |= (config_entity->password_auth) ? SFTPS_AUTH_PASSWORD : 0;
