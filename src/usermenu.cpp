...
static char *
test_condition (const WEdit * edit_widget, char *p, bool * condition)
{
    char arg[256];
    ...
    for (; *p != '\n' && *p != '&' && *p != '|'; p++)
    {
        ...
        *p |= 0x20;
        ...
        case 't':
            p = extract_arg (p, arg, sizeof (arg));
            *condition = panel != nullptr && test_type (panel, arg);
            break;
        case 'x':              /* executable */
            {
                struct stat status;
...
static void
debug_out (char *start, char *end, bool condition)
{
...
static char *
test_line (const WEdit * edit_widget, char *p, bool * result)
{
    char operator_;
    ...
    while (*p != '\0' && *p != '\n')
    {
        ...
        /* Add one debug statement */
        debug_out (&operator_, nullptr, *result);

    }                           /* while (*p != '\n') */
    /* Report debug message */
    debug_out (nullptr, nullptr, true);
...
static void
execute_menu_command (const WEdit * edit_widget, const char *commands, bool show_prompt)
{
    FILE *cmd_file;
    int cmd_file_fd;
    ...
    if (run_view)
    {
        mcview_viewer (vfs_path_as_str (file_name_vpath), nullptr, 0, 0, 0);
    ...
    else
    {
        ...
        cmd = g_strconcat ("/bin/sh ", vfs_path_as_str (file_name_vpath), (char *) nullptr);
...
{
    WPanel *panel = nullptr;
    char *(*quote_func) (const char *, bool);
...
/* --------------------------------------------------------------------------------------------- */
/**
 * If edit_widget is nullptr then we are called from the mc menu,
...
user_menu_cmd (const WEdit * edit_widget, const char *menu_file, int selected_entry)
{
    ...
    if (!g_file_get_contents (menu, &data, nullptr, nullptr))
    {
    ...
    for (menu_lines = col = 0; *p != '\0'; str_next_char (&p))
    {
        if (menu_lines >= menu_limit)
        {
            ...
            char **new_entries = static_cast<char **> (g_try_realloc (entries, sizeof (new_entries[0]) * menu_limit));
