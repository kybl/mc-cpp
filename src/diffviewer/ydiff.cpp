...
cvt_mgeta (const char *src, size_t srcsize, char *dst, int dstsize, int skip, int ts,
           bool show_cr, GArray * hdiff, diff_place_t ord, char *att)
{
    int sz = 0;
...
static int
cvt_fget (FBUF * f, off_t off, char *dst, size_t dstsize, int skip, int ts, bool show_cr)
{
    int base = 0;
    int old_base = base;
            ...
            for (i = 0; i < dview->a[DIFF_LEFT]->len; i++)
            {
                ...
                if (p->line && q->line && p->ch == CHG_CH)
                {
                    ...
                    if (h != nullptr)
                    {
                        ...
                        runresult =
                            hdiff_scan (static_cast<const char *> (p->p), p->u.len, static_cast<char *> (q->p), q->u.len, HDIFF_MINCTX, h,
                                        HDIFF_DEPTH);
                        if (!runresult)
                        {
                            g_array_free (h, true);
...
dview_set_codeset (WDiff * dview)
{
    ...
    if (encoding_id != nullptr)
    {
        ...
        dview->utf8 = (bool) str_isutf8 (encoding_id);
    }
}
...
#ifdef HAVE_CHARSET
                    if (dview->utf8)
                        k = dview_str_utf8_offset_to_pos (static_cast<const char *> (p->p), width);
                    else
#endif
                        k = width;

                    cvt_mgeta (static_cast<const char *> (p->p), p->u.len, buf, k, skip, tab_size, show_cr,
...
#endif
                    k = width;
                cvt_mget (static_cast<const char *> (p->p), p->u.len, buf, k, skip, tab_size, show_cr);
            }
            else
...
{
    bool show_numbers, show_symbols;
    int tab_size;
