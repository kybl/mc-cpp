...
book_mark_find (WEdit * edit, long line)
{
    ...
    for (p = edit->book_mark; p != nullptr; p = p->next)
    {
        if (p->line > line)
    ...
    for (p = edit->book_mark; p != nullptr; p = p->prev)
    {
        if (p->next != nullptr && p->next->line <= line)
...
book_mark_query_color (WEdit * edit, long line, int c)
{
    if (edit->book_mark != nullptr)
    {
        ...
        for (p = book_mark_find (edit, line); p != nullptr; p = p->prev)
        {
            if (p->line != line)
                return false;
            if (p->c == c)
...
book_mark_clear (WEdit * edit, long line, int c)
{
    ...
    for (p = book_mark_find (edit, line); p != nullptr; p = q)
    {
        q = p->prev;
    ...
    if (edit->book_mark->line == -1 && edit->book_mark->next == nullptr)
...
book_mark_flush (WEdit * edit, int c)
{
    ...
    while (edit->book_mark->prev != nullptr)
        edit->book_mark = edit->book_mark->prev;

    for (q = edit->book_mark->next; q != nullptr; q = p)
    {
        p = q->next;
        if (q->c == c || c == -1)
        {
    ...
    if (edit->book_mark->next == nullptr)
        MC_PTR_FREE (edit->book_mark);
...
{
    if (edit->serialized_bookmarks != nullptr)
    ...
    if (edit->book_mark != nullptr)
    {
        ...
        for (p = book_mark_find (edit, 0); p != nullptr; p = p->next)
