...
spell_available (void)
{
    ...
    if (!g_module_symbol (spell_module, "new_aspell_speller", reinterpret_cast<gpointer *> (&mc_new_aspell_speller)))
        goto error_ret;

    if (!g_module_symbol (spell_module, "aspell_error_number", reinterpret_cast<gpointer *> (&mc_aspell_error_number)))
        goto error_ret;
    ...
    if (!g_module_symbol (spell_module, "aspell_speller_error", reinterpret_cast<gpointer *> (&mc_aspell_speller_error)))
        goto error_ret;

    if (!g_module_symbol (spell_module, "aspell_error", reinterpret_cast<gpointer *> (&mc_aspell_error)))
        goto error_ret;

    if (!g_module_symbol (spell_module, "to_aspell_speller", reinterpret_cast<gpointer *> (&mc_to_aspell_speller)))
        goto error_ret;

    if (!g_module_symbol (spell_module, "aspell_speller_check", reinterpret_cast<gpointer *> (&mc_aspell_speller_check)))
        goto error_ret;

    if (!g_module_symbol
        (spell_module, "aspell_speller_suggest", reinterpret_cast<gpointer *> (&mc_aspell_speller_suggest)))
        goto error_ret;

    if (!g_module_symbol
        (spell_module, "aspell_word_list_elements", reinterpret_cast<gpointer *> (&mc_aspell_word_list_elements)))
        goto error_ret;
    ...
    if (!g_module_symbol
        (spell_module, "aspell_config_replace", reinterpret_cast<gpointer *> (&mc_aspell_config_replace)))
        goto error_ret;

    if (!g_module_symbol (spell_module, "aspell_error_message", reinterpret_cast<gpointer *> (&mc_aspell_error_message)))
        goto error_ret;

    if (!g_module_symbol
        (spell_module, "delete_aspell_speller", reinterpret_cast<gpointer *> (&mc_delete_aspell_speller)))
        goto error_ret;

    if (!g_module_symbol (spell_module, "delete_aspell_config", reinterpret_cast<gpointer *> (&mc_delete_aspell_config)))
        goto error_ret;
    ...
    if (!g_module_symbol
        (spell_module, "aspell_config_retrieve", reinterpret_cast<gpointer *> (&mc_aspell_config_retrieve)))
        goto error_ret;

    if (!g_module_symbol
        (spell_module, "aspell_word_list_size", reinterpret_cast<gpointer *> (&mc_aspell_word_list_size)))
        goto error_ret;
