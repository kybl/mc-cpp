...
/* search and replace: */
bool search_create_bookmark = false;

/* queries on a save */
bool edit_confirm_save = true;

/* whether we need to drop selection on copy to buffer */
bool option_drop_selection_on_copy = true;
...
edit_collect_completions (WEdit * edit, off_t word_start, gsize word_len,
                          char *match_expr, GString ** compl_, gsize * num)
{
    gsize len = 0;
    gsize max_len = 0;
    gsize i;
    ...
    while (mc_search_run (srch, (void *) &esm, start + 1, last_byte, &len))
    {
        ...
        {
            if (strncmp
                ((char *) &compl_[i]->str[word_len],
                 (char *) &temp->str[word_len], MAX (len, compl_[i]->len) - word_len) == 0)
            {
                GString *this_ = compl_[i];
...
#endif
        compl_[(*num)++] = g_string_new_len (temp->str, temp->len);
        start += len;
...
void
edit_replace_cmd (WEdit * edit, bool again)
{
...
void
edit_search_cmd (WEdit * edit, bool again)
{
    if (edit == nullptr)
        return;
...
edit_save_block (WEdit * edit, const char *filename, off_t start, off_t finish)
{
    ...
    {
        len = finish - start;
        unsigned char *buf = static_cast<unsigned char *> (g_malloc0 (TEMP_BUF_LEN));
        while (start != finish)
        {
            off_t end = MIN (finish, start + TEMP_BUF_LEN);
...
edit_complete_word_cmd (WEdit * edit)
{
    ...
    off_t word_start = 0;
    GString *match_expr;
    GString *completions[MAX_WORD_COMPLETIONS];       /* completions */
    ...
    max_len =
        edit_collect_completions (edit, word_start, word_len, match_expr->str, (GString **) & completions,
                                  &num_compl);
    ...
    if (num_compl > 0)
    {
        ...
        if (num_compl == 1)
            edit_complete_word_insert_recoded_completion (edit, completions[0]->str, word_len);
    ...
    for (i = 0; i < num_compl; i++)
        g_string_free (completions[i], true);
}
